...
<script type="text/javascript">
    $(function(){
        const link_item = $("#page-tabs a[href='{{ config('app.url').'/events/'.$maincat }}']").parent()[0];
        if(link_item){
...
<div class="row pt-1">
    <div class="col-md-2">
        <ul class="nav flex-column" id="page-tabs">
            <li class="nav-item"><a href="{{ config('app.url') }}/events" class="nav-link gap-2 icon-link {{$maincat == 'company' ? 'active' : ''}}"><img class="rounded-circle" width="30" height="30" src="{{ asset('icon/events.png') }}"> Company Events</a></li>
            <li class="nav-item"><a href="{{ config('app.url') }}/events/holiday" class="nav-link gap-2 icon-link {{$maincat == 'holiday' ? 'active' : ''}}"><img class="rounded-circle" width="30" height="30" src="{{ asset('icon/holiday.png') }}"> Holidays</a></li>
        </ul>
    </div>
